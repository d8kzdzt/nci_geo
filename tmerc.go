	...
	*/
	forward = func(lon, lat float64) (x, y float64, err error) {
		...
		if this.sphere {
			...
			var als = al * al
			var c = this.Ep2 * cos_phi * cos_phi
			var tq = math.Tan(lat)
			var t = tq * tq
			con = 1 - this.Es*sin_phi*sin_phi
			var n = this.A / math.Sqrt(con)
			var ml = this.A * mlfn(e0, e1, e2, e3, lat)

			x = this.K0*n*al*(1+als/6*(1-t+c+als/20*(5-18*t+t*t+72*c-58*this.Ep2))) + this.X0
			y = this.K0*(ml-ml0+n*tq*(als*(0.5+als/24*(5-t+9*c+4*c*c+als/30*(61-58*t+t*t+600*c-330*this.Ep2))))) + this.Y0
